...
| `demo/06_eventstreaming.ipynb` | The outbox table, then the same events on a Kafka topic, offsets, and a replay that rebuilds the target from the log alone. **Stepped through again on 2026-08-16 after the narration was corrected, and it worked as expected**; the outputs are cleared before committing. Not a port — see above. **The counts are not reproducible** — the shop keeps producing, so the topic grows while the notebook is being read. It no longer grows *across* runs: since 2026-08-16 the application empties the topic when it starts, along with truncating its tables, so the log and the database always describe the same run. **Needs the application to have been running for about two minutes**, and the notebook says so at the top: before then the topic holds nothing but `Added customer` and every count is zero. A run inside that window looks broken and is not. |
| `lib/` | Twenty-three functions: `connect`, `invoke`, `write`, `import` and `get_*_data_reader` for `sql`, `ora` and `pg`, `connect`, `write` and `read` for `mdb`, and two `connect`s plus `write`, `read` and `remove` for `kfk`. The six `invoke_*_query` and `write_*_table` functions grew `commit=True` for PhotoService. |
...
| The setup chain | **Rewritten on 2026-08-16 and run start to finish afterwards, with no problems.** `00_check_host.ps1` replaced the Windows `pip install` that used to open `01_setup.ps1`, and every step now announces itself — see "The setup owns WSL2, not your machine" below. The run was from a freshly reinstalled WSL2, this repository's sibling first and then this one. **Oracle takes about two minutes, not fifteen** — measured on the sibling's stack the same day: container created 13:28:41, `DATABASE IS READY TO USE!` at 13:30:23, and no database-creation phase at all, because the image ships a prebuilt XE. The 15 minutes in `wait_for` is a timeout margin and was never a measurement, so do not repeat it as one. Ported to Python, and **run start to finish against a clean install with no errors**, including the Windows half: the `pip install` that used to open the script and the second `06_test_connections.py` that closes it. `01_setup.ps1` is the only remaining PowerShell file, because it is what Windows starts. The Windows run failed the first time it existed, on a port-forwarding race — that is what added the wait in front of it, and the wait has now been through a clean run. Re-run after the build/run split below, and the `docker compose stop` that now ends `01_setup.ps1` works. **The two-repository flow has now been exercised in both directions** (2026-08-15): switching to this stack stopped all seven of the sibling's containers, switching back stopped all eight of these, both exiting 0. Both `06_test_connections.py` runs pass, inside WSL2 and on Windows. |
...
| The outbox is a real transaction, since 2026-08-16 | `photoservice-app.py` writes the `UPDATE order_header` and the `INSERT INTO order_event` with `commit=False` and one `pg_connection.commit()` after both, for payments and for shipments. They used to commit separately, so scenario 6 had to explain the gap rather than the pattern; the owner decided to close it instead. **Do not split them again** — the invariant is checked by `verify/06_eventstreaming.py`, in both directions and with a precondition that there were rows to compare. |
| `docker/photoservice-app.py` | The ported application, running as the `photoservice` service on a stock `python:3.13-slim` image. It mounts `lib/` and installs `pandas`, `psycopg[binary]`, `pymongo` and `confluent-kafka` when the container starts. It is the source of everything the second half of scenario 4 transfers **and of every event demo 6 reads**, so both are empty unless this container is running. `docker compose restart photoservice` is the cheap reset for those two demos — it truncates its own PostgreSQL tables, drops its MongoDB collection, **empties the Kafka topic** and restarts the two-minute clock. The topic reset arrived on 2026-08-16 and it is not cosmetic: the application restarts its ids at 1, so a topic that outlived the tables held several customers with `id = 1` and demo 6's replay died on a primary key violation. See the Kafka section of `DIFFERENCES.md`. Keep that schedule in step with the sibling's `photoservice-app.ps1`. See "Reset the containers" in `README.md`; a `restart` is still the cheap reset for these two demos, but a full `down -v` is no longer the expensive thing it was described as — about two minutes, see "Verifying a change". **It also mounts `/etc/localtime` read only, and that is load-bearing:** `created_at` and `updated_at` are naive local timestamps, and this image's `/etc/localtime` points at UTC, so without the mount the shop stores times two hours behind the wall clock. The sibling mounts the same file for the same reason — the two applications have to agree on what the column means. Entry 19 of its `SIBLING-FINDINGS.md`. |
...
so do them on both sides in one turn, the way the PhotoService schedule is kept in step. Do not treat
either as a finding, and do not do them as a side effect of an unrelated task.
...
1. **Timestamps in the step messages**, so that the actual timing of a run can be read off the
   ...
   once — the Oracle estimate in the state table above.
...
to produce no output while it ran `pip install`, and `04_docker_compose.sh` is silent for about two
minutes while it waits for Oracle. The Windows port wait prints one line per port for
...
- **`docker compose down -v` needs asking first — but the reason changed on 2026-08-16.** This used to
  ...
  Measured on the sibling's stack: **about two minutes**, because the image ships a prebuilt XE and
  there is no database-creation phase at all. The owner's position is that `down -v` is fine when it
  gets a clean lab.

  So the cost is no longer time, it is **whatever state somebody has not saved** — a notebook half
  stepped through, photos loaded into PostgreSQL by scenario 4. Ask, and say what will go.
  `docker compose stop` is still the right thing for merely switching repositories.

  One consequence worth taking advantage of: the init SQL under `docker/` only runs on empty volumes,
  so **editing it and rebuilding now beats writing a migration**, which is what entry 18 of
  `SIBLING-FINDINGS.md` had to do when rebuilding was believed to be expensive.
